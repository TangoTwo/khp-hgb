...
void placeDefaultSetup(chessGame *cG) {
    for (char i = 0; i < (char)cG->getSize(); ++i) {
        cG->placeOnBoard(toCoord(char('A' + i), 2), new pawn(chessman::Colour::WHITE));
        cG->placeOnBoard(toCoord(char('A' + i), 7), new pawn(chessman::Colour::BLACK));
    }

...
BEGIN
  ...
  THEN -- Referenzieren der benutzerdefinierten Exception (Fangen der Exception)
    DBMS_OUTPUT.PUT_LINE('No such department id.');

  WHEN OTHERS
  THEN -- Referenzieren von nicht spezifizierten Exceptions
...
BEGIN
  DELETE FROM employees WHERE manager_id = 1; -- gibt keine Fehlermeldung aus
...
DECLARE
  depId NUMBER;
BEGIN
  SELECT department_id INTO depId FROM employees WHERE employee_id = 1;
...
BEGIN
  ...
  DELETE FROM employees WHERE last_name = 'Max';
